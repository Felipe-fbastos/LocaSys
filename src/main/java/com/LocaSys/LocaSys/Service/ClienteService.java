...
public class ClienteService {
    ...
    public Cliente getId(int id){
        ...
        return repository.findById(id).orElseThrow(() -> new ClienteFoundException("id", id));

    }

    public Cliente getName(String nome){
        return repository.findByNome(nome).orElseThrow(() -> new ClienteFoundException("nome",nome));
    }

    public Cliente getCpf(String cpf){
        return repository.findByCpf(cpf).orElseThrow(() -> new ClienteFoundException("cpf" , cpf));
    }

    public Cliente getCnh(String cnh){
        return repository.findByNroCNH(cnh).orElseThrow(() -> new ClienteFoundException("CNH", cnh));
    }
